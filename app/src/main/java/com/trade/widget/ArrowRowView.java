...
public class ArrowRowView extends AutoFrameLayout {
	...
	private boolean indicatorShow,lineShow;
	/**
	 * @param context
	 * @param attrs
	 */
	public ArrowRowView(Context context, AttributeSet attrs) {
		...
		mTitleTxtv.setTextSize(typeArray.getDimension(R.styleable.ArrowView_titleTextSize, 12));
		mTitleTxtv.setTextColor(typeArray.getColor(R.styleable.ArrowView_titleTextColor, getResources().getColor(R.color.normal_text_color)));

		mSubTitleTxtv = (TextView)view.findViewById(R.id.sub_title_txtv);
		mSubTitleTxtv.setTextSize(typeArray.getDimension(R.styleable.ArrowView_subTextSize, 12));
		mSubTitleTxtv.setTextColor(typeArray.getColor(R.styleable.ArrowView_subTextColor, getResources().getColor(R.color.normal_text_color)));

		imageImgv = (ImageView)view.findViewById(R.id.image_imgv);
		arrowImgv = (ImageView)view.findViewById(R.id.arrow_imgv);
		lineView = (View)view.findViewById(R.id.row_line_view);
...
//		if (drawable != null){
//			marginLeft = context.getResources().getDimension(R.dimen.app_marign_space)
