package com.bear.customerview.bottomtabbar.util;
...
public class TintUtil {
    ...
    public static Drawable setStateListTintDrawable(Drawable normalDrawable,Drawable selectDrawable){
        int[][] states = new int[3][];
        states[0] = new int[]{android.R.attr.state_pressed};
        states[1] = new int[]{android.R.attr.state_selected};
        states[2] = new int[]{};
        ...
        for (int[] state : states) {
            if(i==0||i==2){
                stateListDrawable.addState(state, normalDrawable);
            }else{
                stateListDrawable.addState(state, selectDrawable);
            }
            i++;
        }
        return getStateListDrawable(stateListDrawable, states);
    }
    /**
     * @param drawable       同上
